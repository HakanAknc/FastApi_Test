from fastapi import FastAPI, HTTPException   # TODO SON SÜRÜM 1.0
...
def create_arac(arac: AracBilgileriCreate):
    ...
    db_marka = db.query(Marka).filter(Marka.marka_id == arac.marka_id).first()
    if not db_marka:
        db.close()
        raise HTTPException(status_code=400, detail="Marka ID eksik veya hatalı girdiniz.")
